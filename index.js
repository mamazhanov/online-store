...
// Настройка Cloudinary из переменных Railway
cloudinary.config({
...
// Настройка хранилища: файлы будут сразу лететь в облако
const storage = new CloudinaryStorage({
  cloudinary: cloudinary,
  params: {
    folder: 'kyrgyz_modern_products',
...
<style>
  ...
  .cart-icon { cursor: pointer; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; }
...
app.get('/', async (req, res) => {
  ...
  res.send(`
    ...
    <div class="container" id="collection">
      <div class="grid">${productsHtml || '<p style="text-align:center; width:100%;">Adding new treasures...</p>'}</div>
    </div>
    <div id="cart-sidebar">
      <span class="close-cart" onclick="toggleCart()">×</span>
      <h2 style="margin-top:0;">Your Cart</h2>
      <div id="cart-items"></div>
      <div id="cart-total" style="font-weight:600; font-size:18px; border-top:1px solid #eee; padding-top:20px; margin-top:20px; margin-bottom:20px;">Total: $0</div>
      <input id="cust-name" placeholder="Name">
      <input id="cust-phone" placeholder="WhatsApp">
    ...
    <script>
      ...
      function sendOrder() {
        ...
        if (!name || !phone || cart.length === 0) return alert('Check your details');
        const message = "New Order!\\nName: " + name + "\\nPhone: " + phone + "\\nTotal: $" + cart.reduce((s,i)=>s+i.price,0);
        window.open("https://wa.me/996500002234?text=" + encodeURIComponent(message), '_blank');
      }
    </script>
  `);
});
...
app.post('/admin/add', upload.single('image'), async (req, res) => {
  ...
  // req.file.path теперь содержит прямую ссылку на Cloudinary!
...
app.listen(PORT, () => console.log('Store with Permanent Cloud Storage Ready!'));
