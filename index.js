...
// ТВОИ ОРИГИНАЛЬНЫЕ СТИЛИ (ПОЛНОСТЬЮ)
const style = `
<style>
  ...
  nav { padding: 20px 5%; display: flex; justify-content: space-between; align-items: center; position: fixed; top: 0; left: 0; width: 90%; z-index: 4000; color: #fff; transition: 0.3s; background: transparent; }
  ...
  .hero { height: 100vh; background: linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.2)), url('https://res.cloudinary.com/duiel8ksn/image/upload/v1770544217/ChatGPT_Image_8_%D1%84%D0%B5%D0%B2%D1%80._2026_%D0%B3._15_48_06_nifdei.png'); background-size: cover; background-position: center; display: flex; flex-direction: column; justify-content: center; padding: 0 10%; color: #fff; }
  ...
  .product-title { font-size: 11px; text-transform: uppercase; font-weight: 600; letter-spacing: 1px; margin-bottom: 5px; cursor: pointer; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis; min-height: 32px; }
  .product-price { color: #666; font-family: 'Cormorant Garamond'; font-size: 20px; font-style: italic; margin-bottom: 15px; }
  .buy-btn { border: 1px solid #1a1a1a; background: none; padding: 15px 20px; text-transform: uppercase; font-size: 10px; letter-spacing: 2px; cursor: pointer; width: 100%; transition: 0.3s; font-weight: 600; margin-top: auto; position: relative; overflow: hidden; }
  ...
  @media (max-width: 768px) { .grid { grid-template-columns: repeat(2, 1fr); } .product-detail-content { flex-direction: column; width: 95%; height: auto; max-height: 90vh; overflow-y: auto; } .product-detail-img { width: 100%; height: 40vh; min-height: 300px; } .product-detail-info { padding: 30px 20px; } #cart-sidebar { width: 100%; } }
</style>
`;
    ...
    res.send(`<!DOCTYPE html><html><head><title>Kyrgyz Modern</title><meta name="viewport" content="width=device-width, initial-scale=1">
      ...
      <div id="cart-sidebar">
        ...
        <div class="cart-footer">
          ...
          <button id="main-cart-btn" class="buy-btn" style="background:#1a1a1a; color:#fff;" onclick="handleCheckout()">Checkout</button>
      ...
      <script>
        ...
        // ИСПРАВЛЕННАЯ ФУНКЦИЯ UPDATECART (БЕЗОПАСНАЯ)
        function updateCart() {
          ...
          for (let n in cart) {
            ...
            html += '<div style="display:flex; gap:15px; margin-bottom:20px;">' +
              ...
              '<div>' +
                '<div style="font-size:10px; font-weight:600;">' + n + '</div>' +
                '<div style="display:flex; align-items:center; gap:10px; margin-top:5px;">' +
                  '<button class="qty-btn" onclick="changeQty(\'' + n.replace(/'/g, "\\'") + '\',-1)">-</button>' +
                  i.qty +
                  '<button class="qty-btn" onclick="changeQty(\'' + n.replace(/'/g, "\\'") + '\',1)">+</button>' +
                '</div>' +
              '</div>' +
        ...
        async function handleCheckout() {
          ...
          if (form.style.display === 'none') { form.style.display = 'block'; return; }
          ...
          if (!customer.email || !customer.name) return alert('Fill in Name and Email');

          const response = await fetch('/create-checkout-session', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ cart, customer, total: totalAmount })
          });
...
app.post('/create-checkout-session', async (req, res) => {
  try {
    ...
    const line_items = Object.keys(cart).map(n => ({
      price_data: {
        currency: 'usd',
        product_data: { name: n },
        unit_amount: cart[n].price * 100,
      },
      quantity: cart[n].qty,
    }));
...
app.get('/admin/edit/:id', async (req, res) => {
  ...
  res.send(`${style}<div style="max-width:600px; margin:50px auto; padding:40px; border:1px solid #eee;"><h2>Edit Product</h2><form action="/admin/edit/${p.id}" method="POST" enctype="multipart/form-data"><input name="title_en" value="${p.title_en}" class="input-field"><input name="price" type="number" value="${p.price}" class="input-field"><textarea name="description_en" class="input-field">${p.description_en || ''}</textarea><input name="image" type="file" style="margin:20px 0;"><button type="submit" class="buy-btn">Save</button></form></div>`);
});
